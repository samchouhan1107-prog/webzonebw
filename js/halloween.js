...
function initWebZoneERStudio() {
  ...
  function isERTablet() {
    return (
      window.matchMedia &&
      window.matchMedia("(min-width: 701px) and (max-width: 1100px)").matches
    );
  }
  ...
  function getActiveInventoryFilters() {
    ...
    if (isFaceDetected) {
      ...
      const otherFilters = allFilterConfigs.filter(
        (f) => f.category !== "face",
      );

      return [...faceFilters, ...otherFilters];
    } else {
      const sceneFilters = allFilterConfigs.filter(
        (f) => f.category === "scene" || f.category === "magazine",
      );
  ...
  function renderSmartLensTrack() {
    ...
    if (!visibleLenses.some((f) => f.id === currentFilter)) {
      ...
      if (currentCfg) {
        visibleLenses.unshift(currentCfg);
      }
    }
    ...
    visibleLenses.forEach((config) => {
      ...
      btn.className = `er-lens-bubble ${
        config.id === currentFilter ? "active" : ""
      }`;
      ...
      circle.className = `lens-bubble-circle ${
        config.id === currentFilter ? "active-glow" : ""
      }`;
      ...
      label.textContent = config.name.split(" ")[0];
  ...
  function updateSmartInventoryUI() {
    ...
    const faceCount = allFilterConfigs.filter(
      (f) => f.category === "face",
    ).length;

    const sceneCount = allFilterConfigs.filter(
      (f) => f.category === "scene",
    ).length;
  ...
  function showSwipeToast(icon, title) {
    ...
    if (canvasSwipeIcon) {
      canvasSwipeIcon.textContent = icon;
    }

    if (canvasSwipeText) {
      canvasSwipeText.textContent = title;
    }
  ...
  function selectFilter(filterName, direction = "none") {
    ...
    bubbles.forEach((bubble) => {
      if (bubble.dataset.filter === filterName) {
        ...
        if (circle) {
          circle.classList.remove("active-glow");
        }
      }
    });
    ...
    const isMag = ["time", "wired", "forbes", "vogue", "cyber"].includes(
      filterName,
    );
  ...
  const swipeTargets = [cameraViewport, canvas, touchSlideController].filter(
    Boolean,
  );
  ...
  swipeTargets.forEach((el) => {
    ...
    el.addEventListener(
      "touchmove",
      (e) => {
        if (!isSwiping || !e.touches || e.touches.length === 0) {
          return;
        }
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if ((e.key === "r" || e.key === "R") && randomFilterBtn) {
  ...
  if (snapLensTrack) {
    ...
    snapLensTrack.addEventListener(
      "pointerdown",
      (event) => {
        ...
        lensDragging = false;
        try {
          snapLensTrack.setPointerCapture(event.pointerId);
        } catch (_) {}
      },
      { passive: true },
    );
    ...
    snapLensTrack.addEventListener(
      ...
      (event) => {
        if (
          Math.abs(event.clientX - lensTouchX) > 12 ||
          Math.abs(event.clientY - lensTouchY) > 12
        ) {
          lensDragging = true;
        }
      },
  ...
  function setupCarouselArrows(track, arrowLeft, arrowRight, scrollAmount) {
    ...
    function updateArrowVisibility() {
      ...
      const wrap =
        track.closest(".er-lens-carousel-wrap") || track.parentElement;
      if (wrap) {
  ...
  function applySmartCategory(cat) {
    ...
    smartCatBtns.forEach((btn) => {
      btn.classList.toggle("active", btn.dataset.smartCat === cat);
    });
  ...
  if (effectsSearchInput) {
    effectsSearchInput.addEventListener("input", (e) => {
      ...
      effectCards.forEach((card) => {
        ...
        const name =
          card.querySelector(".effect-name")?.textContent?.toLowerCase() || "";
  ...
  if (flashLightBtn) {
    flashLightBtn.addEventListener("click", () => {
      ...
      showSwipeToast(
        "⚡",
        isStudioLightEnabled ? "Studio Light ON" : "Studio Light OFF",
      );
    });
  }
  ...
  const mobileEffectsToggleBtn = document.getElementById(
  ...
  if (openAllEffectsBtn) {
    openAllEffectsBtn.addEventListener("click", toggleEffectsPanel);
  }

  if (mobileEffectsToggleBtn) {
    mobileEffectsToggleBtn.addEventListener("click", toggleEffectsPanel);
  }
  ...
  if (faceHudToggleFloating) {
    faceHudToggleFloating.addEventListener("click", () => {
      ...
      showSwipeToast(
        "👤",
        showFaceHud ? "Face Reticle ON" : "Face Reticle OFF",
      );
    });
  }
  ...
  function showPermissionAlert(err) {
    ...
    if (
      ...
      title = "Camera Security Restriction";
    ...
    if (permAlertTitle) {
      permAlertTitle.textContent = title;
    }

    if (permAlertBadge) {
      permAlertBadge.textContent = badge;
    }

    if (permAlertMessage) {
      permAlertMessage.innerHTML = message;
    }

    if (permAlertIcon) {
      permAlertIcon.textContent = icon;
    }

    if (permAlertIconWrap) {
      permAlertIconWrap.classList.toggle("error", !!err);
    }
    ...
    permissionAlertModal.classList.add("is-open");
  ...
  function closePermissionAlert() {
    if (permissionAlertModal) {
      permissionAlertModal.classList.remove("is-open");
  ...
  async function pickCameraDeviceId(facingUser) {
    try {
      ...
      const frontCam = cameras.find((device) =>
        /front|user|face|integrated/i.test(device.label),
      );
  ...
  async function requestCameraStream(facingUser) {
    ...
    const deviceId = await pickCameraDeviceId(facingUser);
  ...
  function attachLiveCameraFeed() {
    ...
    setImmersiveCameraMode(isERMobile() || isERTablet());

    video.dataset.facingMode = isFacingUser ? "user" : "environment";

    video.dataset.cameraMode = isFacingUser ? "front" : "back";

    canvas.classList.toggle("camera-mirrored", isFacingUser);

    video.classList.toggle("camera-mirrored", isFacingUser);

    setHighlightStep(6);

    startRenderLoop();

    console.log("[WEBZONE ER] Camera started successfully:", {
  ...
  async function startCamera() {
    ...
    try {
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        ...
        unsupportedError.name = "SecurityError";
      ...
      if (
        ...
        const securityError = new Error("Camera requires HTTPS or localhost.");
      ...
      mediaStream = await requestCameraStream(isFacingUser);
      ...
      console.warn("[WEBZONE ER] Camera access error:", err);
  ...
  function drawDemoBackground(ctx, w, h) {
    ...
    ctx.fillText("DEMO MODE — SELECT A LENS EFFECT", w / 2, h * 0.5);
    ctx.restore();
    ...
    for (let i = 0; i < matrixDrops.length; i++) {
      ...
      for (let c = 0; c < Math.min(d.length, 6); c++) {
        ...
        ctx.fillText(
          matrixChars[(i + c + Math.floor(time * 3)) % matrixChars.length],
          mx,
          my - c * 14,
        );
      }
  ...
  async function updateFaceTracking() {
    ...
    if (
      ...
      video.readyState >= 2 &&
      !isDetectingFace &&
      now - lastFaceDetectTimestamp > 120
    ) {
    ...
    if (faceBox.targetW) {
      faceBox.w += (faceBox.targetW - faceBox.w) * 0.15;
    }

    if (faceBox.targetH) {
      faceBox.h += (faceBox.targetH - faceBox.h) * 0.15;
    }
  }
  ...
  function applyAutoQualityEnhancement(ctx, w, h) {
    try {
      ...
      for (let i = 0; i < d.length; i += 4) {
        ...
        const amount = ((max - avg) / 255) * 1.3;

        if (amount > 0) {
          d[i] += (d[i] - avg) * amount;

          d[i + 1] += (d[i + 1] - avg) * amount;

          d[i + 2] += (d[i + 2] - avg) * amount;
        }
      }
  ...
  function drawCelebritySpotlight(ctx, w, h, time) {
    ...
    halo.addColorStop(0.55, "rgba(168,85,247,.25)");
    ...
    ctx.textAlign = "center";
    ctx.fillText(
      "CELEBRITY SPOTLIGHT • WEBZONEBW",
      cx,
      Math.max(18, cy - ry - 14),
    );
    ctx.restore();
  }
  ...
  function drawGoldenHourGlam(ctx, w, h, time) {
    ...
    [leftCheekX, rightCheekX].forEach((bx) => {
      const blush = ctx.createRadialGradient(bx, cheekY, 0, bx, cheekY, blushR);

      blush.addColorStop(0, "rgba(244, 114, 182, 0.28)");

      blush.addColorStop(0.6, "rgba(251, 113, 133, 0.12)");
  ...
  function drawDesignerAviators(ctx, w, h, time) {
    ...
    function drawAviatorLens(x, y, isRight) {
      ...
      glint.addColorStop(Math.max(0, sheenPos - 0.2), "transparent");

      glint.addColorStop(sheenPos, "rgba(255,255,255,0.45)");

      glint.addColorStop(Math.min(1, sheenPos + 0.2), "transparent");
  ...
  function drawNeonAngelHalo(ctx, w, h, time) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const sy =
        cy + Math.sin(angle) * (haloRy + 6) - Math.sin(time * 2 + i) * 14;
  ...
  function drawVintage90sFilm(ctx, w, h, time) {
    ...
    ctx.fillText("'98 10 31 PM 08:42", w - 24, h - 24);
  ...
  function drawKawaiiAnimeBlush(ctx, w, h, time) {
    ...
    [cx - rx * 0.46, cx + rx * 0.46].forEach((bx) => {
  ...
  function drawLeicaNoirCinema(ctx, w, h, time) {
    ...
    const noirVignette = ctx.createRadialGradient(
      w / 2,
      h / 2,
      Math.min(w, h) * 0.3,
      w / 2,
  ...
  function drawDigitalGlitch(ctx, w, h, time) {
    ...
    for (let i = 0; i < sliceCount; i++) {
      if (Math.sin(time * 12 + i * 4) > 0.4) {
        const sy = Math.floor(
          (Math.sin(time * 8 + i * 2) * 0.5 + 0.5) * (h - 40),
        );

        const sHeight = Math.floor(Math.random() * 20 + 8);

        const shiftX =
          (Math.sin(time * 25 + i * 7) > 0 ? 1 : -1) *
    ...
    ctx.fillText(
      `SIGNAL_CORRUPT: 0x${Math.floor(Math.random() * 0xffff)
        .toString(16)
        .toUpperCase()}`,
      16,
      28,
